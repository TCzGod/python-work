...
#     for i, (x2, y2, w2, h2) in enumerate(faces2):
#         # 确保从图片1中选择足够的面部区域进行处理
#         if i < len(faces1):
...
#             face_region1 = image1[y1:y1 + h1, x1:x1 + w1]
    ...
    resized_face_region1 = cv2.resize(face_region1, (w2, h2))

    # 创建一个只与目标人脸区域相同大小的蒙版
    mask = np.zeros((h2, w2), dtype=np.uint8)

    # 在蒙版上绘制椭圆形状
    center = (w2 // 2, h2 // 2)  # center 是目标人脸区域的中心
    axes = (w2 // 2, h2 // 2)
    cv2.ellipse(mask, center, axes, 0, 0, 360, 255, -1)  # 白色椭圆区域

    # 将mask转换为3通道的格式
    mask_colored = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)

    # 进行无缝融合
    blended_image = cv2.seamlessClone(resized_face_region1, image2, mask_colored, (x2 + w2 // 2, y2 + h2 // 2), cv2.NORMAL_CLONE)
